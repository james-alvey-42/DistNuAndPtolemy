...
int background_ncdm_distribution(
  ...
  /** - b) deal now with case of reading analytical function */
    ...
    */
    ...
    double Amp;
    double Neff=param[0];
    double ystar=param[1];
    double sigma=param[2];
    double TvalR=param[3];

    Amp = Neff * pow(_PI_, 4.) * 7. * pow(4./11., 4./3.) / 45. / 8. / (pow(sigma,2.) * (pow(ystar,2.) + 2.*pow(sigma,2.)) * exp(-pow(ystar,2.)/2./pow(sigma,2.)) + sqrt(_PI_/2.)*ystar*sigma*(pow(ystar,2.)+3.*pow(sigma,2.))*(1.+erf(ystar/sqrt(2.)/sigma)));

    *f0 = 3 * 2 * Amp / pow(0.71611, 4.) / pow(2*_PI_,3) * exp(-pow(q-ystar,2.)/2./pow(sigma,2.)) + 6.0/pow(2*_PI_,3)/(exp(q/TvalR) + 1.);
   }
   else{
   *f0 = 3.0/pow(2*_PI_,3)*(1./(exp(q-ksi)+1.) +1./(exp(q+ksi)+1.));
   }
